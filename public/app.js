...
let roomPlayers = []; // ルーム内プレイヤー一覧（待機画面表示用）
...
function renderWaitingPlayers() {
  ...
  if (!list) return;
  list.innerHTML = '';
  const others = roomPlayers.filter(p => p.id !== myPlayerId);
  if (others.length === 0) return;
  others.forEach(p => {
    const item = document.createElement('div');
    item.className = 'waiting-player-item';
    item.id = 'waiting-player-' + p.id;
    item.innerHTML = `<span class="waiting-player-name">${escapeHTML(p.name)}</span><span class="waiting-dots"><span>.</span><span>.</span><span>.</span></span>`;
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('btn-submit-answer').addEventListener('click', () => {
    ...
    // A: 自分の回答サムネイル表示
    const thumbImg = document.getElementById('my-answer-thumbnail');
    const thumbContainer = document.getElementById('my-answer-preview');
...
socket.on('room-joined', ({ roomId, players, isHost: hostFlag, playerId, hostId }) => {
  ...
  myRoomId = roomId;
  isHost = hostFlag;
  roomPlayers = players.map(p => ({ id: p.id, name: p.name }));
...
socket.on('topic-set', ({ topic, currentRound, totalRounds, submittingStartedAt: startedAt, timerDuration }) => {
  ...
  const thumbContainerReset = document.getElementById('my-answer-preview');
  if (thumbContainerReset) thumbContainerReset.style.display = 'none';
  const waitingListReset = document.getElementById('waiting-player-list');
  if (waitingListReset) waitingListReset.innerHTML = '';
...
socket.on('all-submitted', ({ players }) => {
  ...
  if (players) roomPlayers = players.map(p => ({ id: p.id, name: p.name }));
  document.querySelectorAll('.waiting-player-item').forEach(item => {
    const dots = item.querySelector('.waiting-dots');
...
socket.on('player-disconnected', ({ playerName, players }) => {
  roomPlayers = players.filter(p => !p.disconnected).map(p => ({ id: p.id, name: p.name }));
